...
namespace mithep {
  ...
  public:
    ...
    UInt_t NReweightScaleFactors() const { return fReweightScaleFactors.size(); }
    Double_t ReweightScaleFactor(UInt_t idx) const { return fReweightScaleFactors.at(idx); }
    UInt_t NScales() const { return fScales.size(); }
    ...
    void ClearScales() { fScales.clear(); }
    ...
    void AddLHEComment(char const* c) { fLHEComments.push_back(c); }
  ...
  protected:
    ...
    Bool_t fHasLHEInfo;
    std::vector<Double32_t> fReweightScaleFactors; // alternative event weights normalized to original XWGTUP
